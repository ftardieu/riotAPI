...
class SummonerMatchItem extends Component {
    ...
    getSummonerInfoMatch = async ( gameId) => {
        const response = await api.getSummonerMatch(gameId)
    ...
    getTeam = async ( gameId,  sumMatchItemInfo) => {
        ...
        for (var i = sumMatchItemInfo.participants.length - 1; i >= 0; i--) {
            ...
            maxDamage = Math.max(sumMatchItemInfo.participants[i].stats.totalDamageDealtToChampions, maxDamage)
        ...
        items  , csNumber, csNumberNeutral , sumTeam1Kills , sumTeam2Kills , isTeam1 , wardsPlaced  ,wardsKilled , visionWardsBoughtInGame, championName, maxDamage })
    }
    ...
    render(){
        const { team1 , team2 , gameId , sumMatchItemInfo, isSummonerWin , summonerIcon , summonerSpellName1 , summonerSpellName2  , perk , perkSubStyle , kills ,deaths ,assists , champLevel ,
         items , csNumber , csNumberNeutral , isTeam1 , sumTeam1Kills , sumTeam2Kills  , wardsPlaced , wardsKilled  , visionWardsBoughtInGame, championName , toggleMatch, maxDamage} = this.state
        if (sumMatchItemInfo) {
                ...
                <div className = { (isSummonerWin ? 'gameWin' : 'gameLose') + ' matchItem' } data-game-id = {gameId} >
                                    ...
                                    <div>
                                        <OverlayTrigger placement="right" overlay={championTooltip}>
                                            <div className ='summonerChamp'>
                                                <img alt ='championIcon' className ="summonerIcon" src = {summonerIcon} />
                                            </div>
                                        </OverlayTrigger>
                                        <div className ="summonerSpell">
                                            <div className ='spell'>
                                                <img alt ='summonerSpell1' className ="summonerSpell" src = {summonerSpellName1} />
                                            </div>
                                            <div className ='spell'>
                                                <img alt ='summonerSpell2' className ="summonerSpell" src = {summonerSpellName2} />
                                            </div>
                                        </div>
                                        <div className='summonerRune'>
                                            <div className ='rune'>
                                                <img alt ='summonerRune1' className ="summonerRune" src = {perkImg} />
                                            </div>
                                            <div className ='rune'>
                                                <img alt ='summonerRune2' className ="summonerRune" src = {perkSubStyleImg} />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                 ...
                 {toggleMatch ?
                    <div className='col-xs-12'>
                        <SummonerMatchItemInfo key = {gameId + 'team1Info'} team = {team1} id ={this.props.id} gameId = {gameId} datas = { sumMatchItemInfo } maxDamage = { maxDamage } info = 'team1' />
                        <SummonerMatchItemInfo key = {gameId + 'team2Info'} team = {team2} id ={this.props.id} gameId = {gameId} datas = { sumMatchItemInfo } maxDamage = { maxDamage } info = 'team2' />
                    </div> : null}
            </div>
        )
    }
